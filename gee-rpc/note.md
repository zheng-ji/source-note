...
bufio.NewWriter(conn) , 为了防止阻塞而创建的带缓冲的 Writer
...
func (server *Server) handleRequest(cc codec.Codec, req *request, sending *sync.Mutex, wg *sync.WaitGroup) {
	...
	// body interface{}, 用 reflect.ValueOf 来操作
...
func (server *Server) Accept(lis net.Listener) {
	for {
		...
		go server.ServeConn(conn) // 这里用协程， go accept serveconn
	}
}
...
func (server *Server) serveCodec(cc codec.Codec) {
	...
	for {
		...
		wg.Add(1)
		go server.handleRequest(cc, req, sending, wg) // 这里继续用协程， go handle Request
	}
	wg.Wait()
	_ = cc.Close()
}
